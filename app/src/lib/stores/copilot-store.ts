 ...
 *
 * @param commitMessageRules Human-readable rule descriptions (as produced
 *   by the repo-rules layer) to instruct the model about. When empty or
 *   omitted, the base system prompt is returned unchanged.
 */
export function buildCommitMessageSystemPrompt(
  commitMessageRules?: ReadonlyArray<string>
): string {
  if (!commitMessageRules || commitMessageRules.length === 0) {
    return CommitMessageSystemPrompt
  }

  const bullets = commitMessageRules.map(r => `- ${r}`).join('\n')
...
export class CopilotStore extends BaseStore {
   ...
   * @param commitMessageRules Optional list of human-readable repository
   *   commit-message rule descriptions. When non-empty, they are appended
   *   to the system prompt so the generated message is more likely to
   *   satisfy the rules github.com will evaluate on push.
